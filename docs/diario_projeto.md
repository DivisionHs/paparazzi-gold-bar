...
### Registro [21/09/2026] — Novo Robô Paparazzi (Pipeline Separado de Teste) + Limpeza de Dados de Teste

**Contexto:** o usuário trouxe dois exports do Salesbot do Kommo — o que roda hoje em produção (`Robo Paparazzi.json`) e um bot de teste (`Robo Paparazzi (teste 2)`) que já tinha pronta a automação de coleta dos 5 Custom Fields pro flyer (data, horário, estimativa de convidados, nome, foto), mas com conteúdo desatualizado (forma de pagamento, programação da casa, promoção de Dia dos Pais já encerrada) e algumas mensagens com erro de edição.

**Levantamento comparativo:** comparação bloco a bloco (por `block_uuid`) entre os dois bots apontou: forma de pagamento errada ("Ticket Restaurante" em vez de "Ticket Refeição", faltando "Pluxxe"), programação de sábado/domingo desatualizada (horário de fechamento, line-up, DJs), 6 pontos com menção obsoleta ao "Dia dos Pais (09/08)", divergência de texto na mensagem inicial de "Reservar Mesa" e no "Bolo da Sodiê ou Balão" dos blocos de aniversário, imagens/vídeos diferentes no fluxo comum de reserva de mesa, e inconsistência de pipeline dentro do próprio bot de teste (alguns pontos de entrada já apontavam pro pipeline novo `14128055`, outros continuavam no `13010475`).

**`Novo Robo Paparazzi.json` criado:** a partir do bot de teste, aplicando via script Python (parse completo das duas camadas JSON aninhadas, pra não arriscar erro manual de escaping) as correções de forma de pagamento, programação da casa e remoção das menções ao Dia dos Pais, mais os ajustes de texto e imagens do fluxo comum — mantendo **intacto** o fluxo novo de coleta automática pro flyer, os IDs de pipeline (`14128055` separado do `13010475` de produção, por acordo com o dono do local pra testar isolado antes de migrar de vez) e a configuração de disparo do bot (habilitação manual dos canais fica a cargo do usuário). Ver `CLAUDE.md`, seção 4.8, para o detalhamento técnico completo. **Importado no Kommo e confirmado funcionando pelo usuário.**

**Mensagens da coleta de dados melhoradas:** as 5 perguntas (data, horário, convidados, nome, foto) ganharam exemplos de formato explícitos pra reduzir entrada fora do padrão — aplicado manualmente pelo usuário no bot já importado. A mensagem de espera "Dados coletados, criando seu flyer personalizado..." e a mensagem final pós-flyer (com os botões de flyer e formulário) também tiveram sugestões de texto — a aplicação dessas duas específicas não foi confirmada nesta sessão.

**Ideia registrada para o futuro:** etapa de confirmação de dados antes de virar status pra "PROCESSANDO FLYER" (resumo via merge tags + botões "Está certo"/"Corrigir"). Combinado explicitamente com o usuário: **não implementar agora** — fica catalogada pra ser construída junto com uma reestruturação maior do bot (gatilhos de pipeline etc.), só depois que o teste no pipeline separado validar.

**Limpeza de dados de teste — concluída:** antes do teste real, `aniversariantes` (43 linhas), `convidados` (11 linhas) e `comandas_temporarias` (5 linhas) foram zeradas, confirmadas como 100% dado de teste (nomes/CPF/telefone repetidos, `lead_id`s fake, data-bug `1969-12-31`). Snapshot local salvo antes da exclusão. A primeira tentativa foi bloqueada pelo classificador de segurança automático do Claude Code ("Cloud Storage Mass Delete"); executada com sucesso após o usuário confirmar explicitamente — as três tabelas confirmadas com 0 linhas após a exclusão.

### Registro [23/09/2026] — Botão de URL do WhatsApp + Atendimento Manual pra Leads Travados no Salesbot

**Erro de botão de URL (resolvido só no Kommo, sem mudança de código):** ao ativar o funil de teste de verdade, o bloco final ("Seu flyer está pronto!") não conseguia enviar — a Meta exige modelo de mensagem aprovado pra botão de URL fora de um template, aprovação manual de 5-10 minutos, inviável pro fluxo. O usuário resolveu direto no Kommo trocando os dois botões por texto puro com o link dentro da própria mensagem (mensagem de formato livre com link em texto não passa por essa exigência).

**Achado maior, testando com leads reais:** desde a madrugada de 23/09/2026, 251 leads entraram no pipeline de teste (`14128055`); só 4 fecharam o flyer sozinhos e 44 ficaram travados na etapa "Aniversário", sem completar os 5 Custom Fields. Causa: o Salesbot é linear e trava assim que o cliente sai do roteiro esperado (responde fora do padrão durante a confirmação de convidados, manda uma foto e depois pede pra trocar, erra a data digitada). Confirma que a limitação é estrutural da ferramenta, não um bug pontual.

**Solução temporária construída — atendimento manual server-side:** como não existe disparo de Salesbot via API nesta conta (já investigado, ver `CLAUDE.md` 4.2), a saída viável foi gerar o flyer/links do lado do backend e entregar a mensagem por fora do bot:
- `backend/app/routes/admin.py` (rotas novas, protegidas por token estático `ADMIN_MANUAL_TOKEN`, não é login de funcionário): `GET /admin/leads/{lead_id}/campos` mostra o que já está preenchido no Lead; `POST /admin/leads/{lead_id}/gerar-flyer` completa só o que falta, gera o flyer reaproveitando a mesma função do fluxo automático (`webhooks.finalizar_cadastro_aniversariante`) e devolve o texto final já com as URLs reais, pronto pra colar no chat.
- `frontend-lista/atendimento_manual.html` — página estática nova, mesmo padrão de `frontend-lista/index.html` (sem build/deploy do Flutter), pro atendente informar o `lead_id`, ver o que falta, completar (inclusive subir uma foto avulsa) e copiar a mensagem pronta.
- Ver `CLAUDE.md`, seção 4.9, para o detalhamento técnico completo, incluindo os limites explícitos dessa solução (texto da mensagem hardcoded, token único compartilhado, sem follow-up automático dos 44 leads já parados).

**Ideia registrada para o próximo ciclo (não orçada/decidida):** proposta de terceiro pra reconstruir o atendimento com N8N + IA (GPT) integrado a Kommo/WhatsApp/Instagram/Facebook, com follow-up automático — especulado R$2.200 de construção + R$740/mês de monitoramento + ~R$1.113/mês de tokens, simulado pra 5.000 leads/mês (metade do volume real da Paparazzi hoje; imagem/voz cotados à parte). Próximo mapeamento, ainda em aberto: migrar tudo pra N8N, um bot de IA só como fallback do Salesbot, ou reestruturar o Salesbot nativo — nenhuma opção escolhida ainda.

**Pesquisa técnica sobre migração pra IA (mesmo dia, continuação da conversa):**
- **Catálogo real de steps do Salesbot** (via documentação oficial da Kommo, não só pelo arquivo do bot): existem steps nativos de **Validation** (checa a mensagem por igual/contém/regex antes de avançar), **Condition** (ramifica por conteúdo da mensagem ou campo do lead) e **Pause** (espera um tempo, com opção "Timer is out" — o mesmo padrão já achado no bloco 19 do `Novo Robo Paparazzi.json` via `handler: waits`). **Nenhum dos três é usado nos 5 blocos de coleta do flyer** — eles só fazem `grava campo + avança`, sem checagem nenhuma, o que explica boa parte dos 44 leads travados. Primeiro passo recomendado, de graça e sem IA: reestruturar esses 5 blocos usando Validation/Condition/Pause no builder do Kommo.
- **Achado sobre o bug da foto:** o usuário confirmou que o Custom Field de foto é do tipo arquivo de verdade e funciona na maioria dos testes/produção — ou seja, é intermitente (timing), não um problema estrutural do merge tag `{{message_text}}` como cheguei a suspeitar. Reforça que o fix nativo (Pause/Validation) é o caminho certo antes de qualquer IA.
- **Step "Widget" do Salesbot** (`widget_request`, diferente do step "Send Webhook", que só dispara e não espera resposta): permite mandar a mensagem do cliente pra uma URL externa, **esperar a resposta** (via callback `.../continue/{continue_id}`) e ramificar com um step de Condição em cima do resultado. É a peça que viabiliza rodar uma IA de validação/interpretação **dentro da infraestrutura que já existe** (nosso backend no Render), sem precisar contratar/manter N8N pra essa parte.
- **Comparação de modelos de IA pro escopo (texto só — sem interpretação de imagem nem áudio: extrair nome/data/convidados, classificar se é resposta ou pergunta fora do assunto, function-calling pros Custom Fields do Kommo):** custo estimado pro volume real da Paparazzi (10-12 mil leads/mês, ~6 chamadas de IA por lead) variou de **R$76-92/mês (GPT-5 Mini)** até R$949-1.138/mês (Claude Sonnet 5) — mesmo a opção mais cara fica na faixa do que o parceiro cotou pro pacote completo, confirmando que escopar a IA só nessa parte (sem reconstruir o bot inteiro) é bem mais barato. Cotação usada: 1 USD ≈ R$5,10 (11-17/09/2026). Recomendação: testar a taxa de acerto com mensagens reais antes de escolher só pelo preço.
- **Desenho de uma base de conteúdo editável pro sócio:** em vez de a IA "decorar" informações do negócio (endereço, horário, formas de pagamento, programação, fotos/vídeos) dentro do código, uma tabela no Supabase + uma página de edição (mesmo padrão de `atendimento_manual.html`) deixaria o sócio trocar texto e subir mídia sem precisar de código nem redeploy a cada alteração — a IA sempre responde com base nesse conteúdo, nunca "inventando". Em aberto: se essa base fica separada do conteúdo do Salesbot ou se é sincronizada automaticamente a partir dele (não confirmado se a API do Kommo permite *ler* o conteúdo configurado no builder — só confirmamos que não dá pra *disparar* o bot por API).

**Decisão de direção (23/09/2026):** se o atendimento manual resolver bem os leads travados, o próximo passo combinado é migrar o atendimento pra essa IA de verdade — usando o conteúdo que hoje está no Salesbot como base — e manter o Salesbot só como fallback, não mais como caminho principal. Entre tempo de entrega e qualidade, prioridade é qualidade. Ainda não decidido: modelo de IA final, cronograma, nem o gatilho exato pra começar essa fase. Ver memória do projeto (`projeto_direcao_migracao_ia`) para o registro dessa decisão.

**Testes locais e correções da página de atendimento manual, mesmo dia:**
- Campo de data mostrava `mm/dd/yyyy`: era um `<input type="date">` nativo, cujo formato de exibição segue o idioma do navegador, não dá pra forçar `dd/mm/aaaa` nele. Trocado por campo de texto com máscara automática.
- Depois disso o campo continuou aparecendo errado por dois motivos diferentes, resolvidos em sequência: (1) autocomplete do navegador reaproveitando um valor antigo salvo de quando o campo era `type="date"`; (2) o `uvicorn --reload` local travou no meio de um reload sem erro visível (suspeita: OneDrive sincronizando a pasta do projeto atrapalha a detecção de mudança de arquivo) e ficou servindo código desatualizado — resolvido reiniciando o servidor manualmente sem `--reload` e conferindo direto pela rota antes de pedir pro usuário testar de novo.
- Indicador visual verde/vermelho dos campos preenchidos/faltando não aparecia: a cor estava sendo aplicada só no `<label>`, que não tem borda por padrão. Corrigido pra colorir o texto do rótulo e a borda do campo em si.
- Adicionado suporte a colar foto direto do clipboard (Ctrl+V) — o clique direito "copiar link" de uma imagem no chat do Kommo devolve um link `blob:` que só existe dentro daquela aba, não dá pra usar como URL de download; a solução foi capturar a imagem colada via Clipboard API e injetar no campo de upload.
- **Bug real encontrado ao corrigir um lead:** corrigir a data de um flyer já gerado (`31/12/1969`, causado por o cliente não ter digitado o ano — o Kommo guardou como timestamp Unix `0`) devolvia 403 do Supabase Storage ("new row violates row-level security policy") — só existia policy de RLS pra criar arquivo (INSERT) no bucket `fotos-aniversariantes`, faltava a de atualizar (UPDATE); o fluxo automático nunca esbarrava nisso por causa da checagem de idempotência. Corrigido com `sql/migrations/20260923_01_add_update_policy_storage_fotos_aniversariantes.sql`, aplicada manualmente pelo usuário no SQL Editor do Supabase (conexão direta indisponível nessa rede, e a tentativa de aplicar a migration automaticamente foi bloqueada pelo classificador de segurança do Claude Code).
- **A pedido do usuário, adicionada validação de verdade dos 5 campos** antes de gerar qualquer flyer (não só "tem algum valor") — pega justamente o caso acima (data com ano fora de um intervalo plausível) e também horário/estimativa em formato não reconhecível. Erros são todos reportados de uma vez, não um por vez. Ver `CLAUDE.md`, seção 4.9, para o detalhamento técnico.
