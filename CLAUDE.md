...
- **`GET /convidados/lista/{lead_id}` também devolve `utilizado` por convidado (decisão de 16/09/2026):** pedido do usuário — além do total confirmado no formulário (`_convidados.length`, já existia), mostrar quantos já entraram de fato. Mesma coluna `utilizado` marcada por `POST /convidados/validar-qr` na Portaria (ver 4.5). `ConvidadosListaScreen` calcula a contagem client-side (`_convidados.where((c) => c.utilizado).length`) e mostra as duas estatísticas lado a lado no cabeçalho; cada linha da lista também ganhou um indicador visual (ícone verde + "entrou") pros convidados que já bipuram entrada.
